...
import { requireGestorPFWithScope, ok, badRequest } from "@/lib/api-helpers";
import { validarCPF } from "@asa/shared";

export async function GET(req: NextRequest) {
  try {
    const { session, error } = await requireGestorPFWithScope();
    ...
    // Check if CPF already exists as partner
    ...
    // Check if CPF is a client (cannot be both)
    ...
    if (ehCliente) {
      return ok({
        valid: false,
        message: "Este CPF já é um cliente no sistema",
      });
    }
